...
if __name__ == '__main__':
    ...
    parser.add_argument("-c", "--check_teams", type=str, nargs='*', default="")
    args = parser.parse_args()
    if len(args.check_teams) == 8 and args.map[0]:
        print(check_teams(args.check_teams, args.map[0]))
    elif len(args.balance) == 8 and args.map[0]:
        print(balance_teams(args.balance, args.map[0]))
    elif args.local and args.local[0]:
